...
public class PlayerControllerMP
{
    ...
    // Start ClientBase
    // This is strictly for Aura / when we need to continuously block a sword
    public void fakeBlock() {
        ItemStack held = mc.thePlayer.inventory.mainInventory[mc.thePlayer.inventory.currentItem];
        if (held == null || !(held.getItem() instanceof ItemSword)) {
            return;
        }

        ItemStack usedItemStack = held.useItemRightClick(mc.theWorld, mc.thePlayer);
        if (usedItemStack != null && usedItemStack.getItem() instanceof ItemSword) {
            mc.thePlayer.inventory.mainInventory[mc.thePlayer.inventory.currentItem] = usedItemStack;
        }
    }
